...
## Season Analysis 
First of all, the data exploration started with the question of "is there a home advantage?". By calculating the percentage of wins by locations during regular season. How teams perform during regular seasons affect their chances of getting 

Since games are played at different locations for both regular seasons and March Madness

Before looking into NCAA Tournament results, we looked at the results from regular seasons. The games are played at different locations from both regular seasons and during the March Madness. The following analysis hopes to find whether or not the locations of the match affect the game result. As a matter of fact, 60% of the winning games happened at the home stadium, which proves that there is a home advantage. 
...
As a group, you will submit files as you would for homework which include a .pdf/.html and .Rmd file. If your data is less than 10MB, you should submit the data too. Otherwise, you should provide a shared link to your actual cleaned R data (using, for example, google drive). Your report must contain the following:
    ...
    Project title - done 
    Group Member Info - done 
    Introduction and literature review (1 - 2 pages) - done
        Data source information (provide a link to your data source) - done
        You must have a comprehensive introduction of the data (no analysis) and the scientific goal - done
