...
for(hyper in hyper_parameter){
  ...
  weighted_mean=function(dat,colnum){
    ...
    c[colnames(sub_train)[colnum+4]]=apply(d,1,,FUN=weighted.mean,w=c(hyper,1-hyper),na.rm=TRUE)
    c=c[,c(-3,-4)]
...
It turned out that weight (0.05,0.95) gives the best result but in case that there was no same match before, I used (0.01,0.09).  
```{r}
  test=read.csv("sub_test.csv")[-1]
  weighted_mean=function(dat,colnum){
    a=dat%>%group_by(TeamID1)%>%summarise(mean(.data[[colnames(dat)[colnum]]],na.rm=TRUE))
    ...
    #The weights are highper  parameter
    #When modeling we tried to adjust this value 
    c[colnames(sub_train)[colnum+4]]=apply(d,1,,FUN=weighted.mean,w=c(0.05,0.95),na.rm=TRUE)
    c=c[,c(-3,-4)]
    test=dplyr::left_join(test,c,by=c("WTeamID"="TeamID1","LTeamID"="TeamID2"))
    return(test)
  }
